...
use bevy::prelude::{Assets, Camera, EventReader, FromWorld, Image, Mat3, MaterialPlugin, MouseButton, PerspectiveCameraBundle, PerspectiveProjection, SpawnSceneCommands, Vec2, Vec4, Windows, World};
...
use bevy::render::render_resource::{BindGroup, BindGroupDescriptor, BindGroupEntry, BindGroupLayout, BindGroupLayoutDescriptor, BindGroupLayoutEntry, BindingResource, BindingType, Buffer, BufferBindingType, BufferInitDescriptor, BufferSize, BufferUsages, CachedComputePipelineId, ComputePipelineDescriptor, Extent3d, PipelineCache, RenderPipelineDescriptor, ShaderStages, SpecializedMeshPipelineError, StorageTextureAccess, TextureDimension, TextureFormat, TextureUsages, TextureViewDimension};
...
fn main() {
    ...
    app.sub_app_mut(RenderApp)
        ...
        .add_system_to_stage(RenderStage::Extract, extract_volume_texture)
...
fn setup(
    ...
    commands.insert_resource(TextureVolume(image));
}
...
#[derive(Deref)]
struct TextureVolume(Handle<Image>);
struct TextureVolumeBindGroup(BindGroup);

// extract the volume texture into a resource in render world
fn extract_volume_texture(mut commands: Commands, image: Res<TextureVolume>) {
    commands.insert_resource(TextureVolume(image.clone()));
}

fn queue_bind_group(mut commands: Commands,
    pipeline: Res<VolumetricFogPipeline>,
    gpu_images: Res<RenderAssets<Image>>,
    volumetric_texture: Res<TextureVolume>,
    render_device: Res<RenderDevice>
) {
    let view = &gpu_images[&volumetric_texture.0];
    ...
    commands.insert_resource(TextureVolumeBindGroup(bind_group));
}
...
impl FromWorld for VolumetricFogPipeline {
    fn from_world(world: &mut World) -> Self {
        ...
        let shader = world
            ...
            .load("shaders/volumetric_fog.wgsl");
